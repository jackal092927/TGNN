"""
Training script for Edge Convolution GNN on triadic closure datasets
"""
...
from edge_conv_gnn import EdgeConvGNN, EdgeGraph
...
def setup_logging(data_name):
    ...
    logging.basicConfig(
        ...
        handlers=[
            logging.FileHandler(f'log/edgeconv_gnn_{data_name}_{int(time.time())}.log'),
...
def evaluate_timestamp_predictions(model, g_df, e_feat, timestamp, all_nodes, logger, balance_ratio=1.0):
    """
    Evaluate model predictions for a specific timestamp
    
    Args:
        model: EdgeConvGNN model
...
def train_edge_conv_gnn(data_name='triadic_medium', num_epochs=100, learning_rate=0.001, 
                       hidden_dim=128, num_layers=3):
    """
    Train Edge Convolution GNN on triadic closure dataset
    """
    logger = setup_logging(data_name)
    logger.info(f"Training Edge Convolution GNN on {data_name}")
    ...
    for epoch in range(num_epochs):
        ...
        for ts in train_timestamps:
            ...
            # Balance sampling (1:2 ratio for training)
            num_positives = len(positive_candidates)
            if num_positives == 0:
                continue
            
            num_negatives_to_sample = min(num_positives * 2, len(negative_candidates))
            ...
            if epoch % 10 == 0:
                ...
                if len(val_aps) > 0:
                    ...
                    if val_ap > best_val_ap:
                        best_val_ap = val_ap
                        torch.save(model.state_dict(), f'models/edgeconv_gnn_{data_name}_best.pt')
    ...
    try:
        model.load_state_dict(torch.load(f'models/edgeconv_gnn_{data_name}_best.pt'))
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Train Edge Convolution GNN')
    ...
    print(f"Training Edge Convolution GNN on {args.data}")
    print(f"Architecture: Edge-centric GNN with {args.num_layers} layers")
    print(f"Focus: Direct triadic closure pattern learning")
    
    # Train model
    results = train_edge_conv_gnn(
        data_name=args.data,
    ...
    print("\n" + "="*60)
    print("FINAL RESULTS (Edge Convolution GNN)")
    print("="*60)
